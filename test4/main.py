...
class GameMonitor:
    ...
    # Горизонтальные свапы (0-89)
        for row in range(10):
            for col in range(9):
                self.valid_actions.append(row * 9 + col)
    
    # Вертикальные свапы (90-179)
        for row in range(9):
            for col in range(10):
    ...
    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            # Предсказание только для валидных действий
    ...
    def perform_action(self, action):
        try:
        # Преобразование action в координаты
            if action < 90:  # Горизонтальные свапы
                row = action // 9
                col = action % 9
                pos1 = (row, col)
                pos2 = (row, col + 1)
            else:  # Вертикальные свапы
                action -= 90
                row = action // 10
                col = action % 10
                pos1 = (row, col)
    ...
    # Бонус за потенциальные комбинации
    ...
    def analyze_potential_moves(self, state):
        # Анализ возможных будущих ходов
        potential = 0
        for r in range(10):
            for c in range(10):
